...
- `downloadModel({ repoId, filename, revision?, destinationDir, onProgress? })` — Node/Desktop default (fs + fetch), downloads if missing, returns local path.
...
- `downloadModelWithAdapter(source, { destinationDir, adapter, onProgress? })` — React Native path with adapter injection.
