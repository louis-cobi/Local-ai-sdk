...
function normalizeParallelStatus(raw: unknown): ParallelStatus {
  const value = (raw ?? {}) as Record<string, unknown>;
  const activeRaw = Array.isArray(value.activeRequests)
    ? value.activeRequests
    : Array.isArray(value.active_requests)
      ? value.active_requests
      : [];
  return {
    enabled: Boolean(value.enabled ?? value.isEnabled ?? value.is_enabled ?? false),
    maxSlots: Number(value.maxSlots ?? value.max_slots ?? value.n_parallel ?? 0),
    queueLength: Number(value.queueLength ?? value.queue_length ?? 0),
    activeRequests: activeRaw.map((item) => {
      const req = item as Record<string, unknown>;
      return {
        requestId: Number(req.requestId ?? req.request_id ?? 0),
        status: String(req.status ?? 'unknown'),
        progress: req.progress == null ? undefined : Number(req.progress),
        queuePosition: req.queuePosition == null ? undefined : Number(req.queuePosition),
      };
    }),
  };
}
  ...
  const parallelApi: ParallelAPI = {
    async completion(req: CompletionRequest, onToken?: (requestId: number, chunk: TokenChunk) => void) {
      ...
      const request = await parallel.completion(
        ...
        onToken
          ? (requestId: number, data: TokenData) => {
    ...
    async getStatus() {
      ...
      return normalizeParallelStatus(await parallel.getStatus());
    },
    async subscribeToStatus(callback: (status: ParallelStatus) => void) {
      const c = requireCtx();
      const parallel = requireFeature(c.parallel, 'parallel status subscriptions');
      return parallel.subscribeToStatus((status) => {
        callback(normalizeParallelStatus(status));
      });
    },
  };
